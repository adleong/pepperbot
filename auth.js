...
async function provider(db, user, clientId, clientSecret) {
    ...
    console.warn(`Attempting to load token for ${user}...`);
    ...
    console.warn(`Loaded token for ${user}.  Token will expire on ${expiry}`)
    return new RefreshableAuthProvider(
        new StaticAuthProvider(clientId, accessToken),
        {
            ...
            onRefresh: async ({ accessToken, refreshToken, expiryDate }) => {
                console.warn(`Refreshing token for ${user}.  Next refresh at ${new Date(expiryDate)}`);
