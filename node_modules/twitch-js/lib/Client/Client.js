...
var Client = /** @class */ (function (_super) {
    ...
    Client.prototype._handleMessage = function (messageEvent) {
        ...
        var messages = [];
        try {
            messages = parsers_1.default(rawMessage, this._options.username);
        }
        catch (error) {
            /**
        ...
        messages.forEach(function (message) {
            ...
            if (utils.isAuthenticationFailedMessage(message)) {
                _this._multiEmit([types_1.ClientEvents.ALL, types_1.ClientEvents.AUTHENTICATION_FAILED], __assign(__assign({}, message), { event: types_1.ClientEvents.AUTHENTICATION_FAILED }));
                _this.disconnect();
            }
            else {
                if (message.command === twitch_1.Commands.PING) {
                    ...
                    _this._multiEmit([types_1.ClientEvents.ALL, types_1.ClientEvents.CONNECTED], __assign(__assign({}, message), { event: types_1.ClientEvents.CONNECTED }));
                }
                else if (message.command === twitch_1.Commands.GLOBALUSERSTATE) {
                    // Handle successful authentications.
                    _this._multiEmit([types_1.ClientEvents.ALL, types_1.ClientEvents.GLOBALUSERSTATE], __assign(__assign({}, message), { event: types_1.ClientEvents.GLOBALUSERSTATE }));
                    if (token && username) {
                        _this._multiEmit([types_1.ClientEvents.ALL, types_1.ClientEvents.CONNECTED], __assign(__assign({}, message), { event: types_1.ClientEvents.CONNECTED }));
                    }
                }
                    ...
                    // Handle RECONNECT.
                    _this._multiEmit([types_1.ClientEvents.ALL, types_1.ClientEvents.RECONNECT], __assign(__assign({}, message), { event: types_1.ClientEvents.RECONNECT }));
                }
                else {
                    _this.emit(types_1.ClientEvents.ALL, message);
                }
            }
        });
        this.emit(types_1.ClientEvents.RAW, rawMessage);
    };
    ...
    Client.prototype._handleClose = function (_closeEvent) {
        this.emit(types_1.ClientEvents.DISCONNECTED);
    };
    Client.prototype._handleHeartbeat = function () {
        ...
        this._reconnectTimeoutId = setTimeout(function () {
            _this.emit(types_1.ClientEvents.RECONNECT);
